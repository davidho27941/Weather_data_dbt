...
{#
    ...
    -----------------------
    Hourly rollup of weather observations: one row per (station_id,
    floor(measure_at, hour)). Each numeric field has avg/max/min plus
    observation counts so consumers can detect partial buckets.

    Re-scans the last `measurements_lookback_days` of source data on each
    incremental run; MERGEs the resulting buckets so partial hours from
    the previous run get overwritten with the now-complete data.
#}
