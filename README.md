...
- **v2 (current)**: GCP-native.
  ...
  - PR #8 — Design decision notes under [`docs/decisions/`](docs/decisions/) for non-obvious choices: STRING-typed bronze sentinels (001), dual-column raw + cleaned staging (002), and a proposal for enforcing dbt contracts on the marts layer (003, `Status: Proposed`). The contract enforcement lands in a separate follow-up PR so the design and the column-by-column implementation get reviewed independently. (004 added later in PR #11; see entry below.)
  ...
  - PR #11 — CWA `Precipitation` / `SunShine` semantics fix: O-A0003-001's `WeatherElement.Now.Precipitation` is daily-cumulative since Asia/Taipei midnight, not a 10-min window as previously assumed. Staging now exposes both the renamed `*_daily_cumulative` columns and a LAG-diff-derived `*_10min_window` column per [decision 004](docs/decisions/004-cwa-precipitation-sunshine-are-daily-cumulative.md); rollup facts switch to `SUM` the derived 10-min column (was silently wrong); `_models.yml` `accepted_range` tests rebase onto the right column with thresholds calibrated to the column's actual semantics.
